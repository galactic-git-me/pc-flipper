...
const createTouchTexture = (): TouchTexture => {
  ...
  return {
    ...
    addTouch,
    update,
    get radiusScale() { return _radiusScale; },
...
const createLiquidEffect = (
  ...
  return new Effect("LiquidEffect", fragment, {
    uniforms: new Map<string, THREE.Uniform<number | THREE.Texture>>([
...
/* ------------------------------------------------------------------ */
/*  Internal three.js state ref type                                    */
/* ------------------------------------------------------------------ */
...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      // ── Random time offset (avoids all instances syncing) ───────────
      const randomFloat = () => {
      ...
      if (noiseAmount > 0) {
        ...
        composer.passes.forEach((p) => { (p as EffectPass).renderToScreen = false; });
      ...
      const animate = () => {
        ...
        uniforms.uTime.value =
          timeOffset + ((performance.now() - startTime) / 1000) * speedRef.current;
        if (liquidEffect)
          liquidEffect.uniforms.get("uTime")!.value = uniforms.uTime.value;
        if (composer) {
          if (touch) touch.update();
          composer.passes.forEach((p) => {
            // effects is private in the postprocessing types — cast through unknown
      ...
      // ── Hot-update uniforms (no reinit needed) ──────────────────────
      const t = threeRef.current!;
